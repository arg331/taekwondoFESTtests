 ...
 *     un proxy con solo el ID. Hibernate lo entiende como "asocia con
 *     este tag existente", no como "inserta uno nuevo".
 *
 *  2. Búsqueda con criterios: el puerto recibe un QuestionCriteria. El
 *     adaptador lo traduce en una combinación de Specifications (filtros
 *     componibles), delegando en QuestionSpecs.
 */
@Repository
public class QuestionRepositoryJpaAdapter implements QuestionRepository {
    ...
    public Question save(Question question) {
        // Resolver los tags del dominio a referencias JPA gestionadas
    ...
    public List<Question> findByCriteria(QuestionCriteria criteria) {
        Specification<QuestionJpaEntity> spec = Specification.allOf(
    ...
    /**
     * Convierte tags del dominio en referencias JPA gestionadas, evitando
     * que Hibernate los trate como entidades nuevas.
     */
    private Set<TagJpaEntity> resolveManagedTags(Set<Tag> domainTags) {
        ...
        for (Tag tag : domainTags) {
            // getReference no toca la BD: crea un proxy con el ID.
            // Si el ID no existe, fallará al hacer flush, no aquí.
